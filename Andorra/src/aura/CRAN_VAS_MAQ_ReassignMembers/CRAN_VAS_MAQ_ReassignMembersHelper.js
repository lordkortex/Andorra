({
			...
			if (component.isValid() && state === "SUCCESS") {
				...
				if(mapSObjectResponse != null){
                    ...
                    if(offices !== undefined){                        
                        ...
                        if (mapSObjectResponse["showUserFilter"] !== undefined){
                        	component.set("v.showUserFilter", mapSObjectResponse["showUserFilter"]);  
                        }
                    }
	...
	filterMembersHelper : function(component,event) {
		...
		if (!$A.util.isEmpty(component.find("offices"))) {
				component.find("offices").set("v.value", "");
		}
		...
		if ($A.util.isEmpty(component.get("v.user")) && component.get("v.showUserFilter")) {
			
            helper.showToast($A.get("$Label.c.CRAN_VAS_MAQ_RM_FilterTitle"),"warning", $A.get("$Label.c.CRAN_VAS_MAQ_RG_UserRequired"));			
			return;
		}
        if ($A.util.isEmpty(component.get("v.campaign"))) {
			
            helper.showToast($A.get("$Label.c.CRAN_VAS_MAQ_RM_FilterTitle"),"warning", $A.get("$Label.c.CRAN_VAS_MAQ_RM_CampaignRequired"));			
			return;
		}
		...
		action.setCallback(this, function(response) {
			...
			if (component.isValid() && state === "SUCCESS") {
				...
				helper.fetchMembers(component, helper, JSON.parse(response.getReturnValue())); 
	...
	fetchMembers: function (component, helper, sObject) {
        ...
        if (helper.$dataTableSelected != "undefined" ||  helper.$dataTable != "undefined") {
            ...
            helper.showToast($A.get("$Label.c.CRAN_VAS_MAQ_RM_FilterTitle"), "warning",$A.get("$Label.c.CRAN_VAS_MAQ_RM_NoMembers") );            
		}
	},
	...
	getUsersByOfficeHelper: function (component,event) {
		...
		var action = component.get("c.getOfficesAndUsers");
		var helper = this;
		console.log("Componente" + component.get("v.user"));
		action.setParams({
            "officeSelected"	: officeSelected,
            "selectedUser" 		: component.get("v.user")
        });
		action.setCallback(this, function(response) {
	...
	fetchUsers: function (component, users) {
		console.log(users);
		if (users === undefined || !users.hasOwnProperty('records')) {
			this.showToast($A.get("$Label.c.CRAN_VAS_MAQ_RM_Title"),"warning",$A.get("$Label.c.CRAN_VAS_MAQ_AG_NoUsers")); 
			var usersOffice= [];
			usersOffice.push({label: "",
					value: "",             			
					selected: true}); 
			if (!$A.util.isEmpty(component.find("users"))) {
				component.find("users").set("v.options", usersOffice);
	...
	reassignMembersToAdvisorHelper: function(component,event) {
		
		var helper 			= this;
		var selectedUser 	= component.find("users").get("v.value"); 
		
		if ($A.util.isEmpty(selectedUser) || $A.util.isEmpty(helper.selectedIds)) {
			this.showToast($A.get("$Label.c.CRAN_VAS_MAQ_RM_Title"),"warning",$A.get("$Label.c.CRAN_VAS_MAQ_RM_NoMembersSelected"));
			return;
		}		
		
        helper.toggleSpinner(component);

        var action = component.get("c.reassignMembersToUser");
               
        action.setParams({
        	"memberIds"	: helper.selectedIds,
	...
	toggleSpinner: function (component) {
		var spinner = component.find("assignMembersSpinner");
